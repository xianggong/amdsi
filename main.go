...
// Register represent a register or a register series
...
func parse(path string) (err error) {
	...
	for scanner.Scan() {
		...
		// Split line by delimiter function
		inputField := strings.FieldsFunc(line, delimiter)
		...
		for i := 0; i < len(inputField); i++ {
			pattern := regexTable["registers"]
			result := parseNamedGroup(pattern, inputField[i])
			if len(result) != 0 {
				fmt.Println(result)
			}
		}
...
func GetInstType(asm string) string {
